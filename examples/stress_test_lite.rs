...
use bevy_3d_physics_lite::{Bevy3dPhysicsLitePlugin, Collider, Damping, Gravity, RigidBody, Speed};
    ...
    let mut unit =
        |size: f32, speed: f32, translation: Vec3| -> (PbrBundle, Commandable, UnitBundle) {
            (
                ...
                UnitBundle::new(Vec3::new(size / 2.0, size / 2.0, size / 2.0)),
            )
        };
...
fn spawn_map(
    ...
    cmds.spawn((
        ...
        Collider {
            cuboid: Vec3::new(MAP_SIZE / 2.0, 0.1, MAP_SIZE / 2.0),
        },
        Ground,
        MapBase,
        Name::new("Map Base"),
...
pub fn drag_select(
    ...
    for (unit_ent, unit_trans) in unit_q.iter() {
        ...
        if in_box_bounds {
            ...
            cmds.entity(unit_ent).remove::<Selected>();
...
fn move_unit(
    ...
    for (mut trans, mut ext_impulse, mut destination) in unit_q.iter_mut() {
        ...
        if let Some(new_pos) = destination.0 {
            ...
            if distance.length_squared() <= 5.0 {
                ...
                ext_impulse.impulse += direction * time.delta_seconds();
