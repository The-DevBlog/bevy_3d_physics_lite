...
fn resolve_aabb_collision(
    ent_a: (&mut Collider, &mut RigidBody, &mut Transform), // Added Quat for rotation
    ent_b: (&mut Collider, &mut RigidBody, &mut Transform), // Added Quat for rotation
) {
    let (collider_a, rigid_body_a, transform_a) = ent_a;
    let (collider_b, rigid_body_b, transform_b) = ent_b;

    let diff = transform_a.translation - transform_b.translation;
    ...
    // Move objects apart along the axis of least penetration
    if !rigid_body_a.fixed {
        transform_a.translation += axis * min_overlap * 0.5;
    }

    if !rigid_body_b.fixed {
        transform_b.translation -= axis * min_overlap * 0.5;
    }
    ...
    // **Apply Torque to Create Rotational Movement**
    let r_a = (transform_a.translation - transform_b.translation).cross(axis); // Calculate torque arm for entity A
    let r_b = (transform_b.translation - transform_a.translation).cross(axis); // Calculate torque arm for entity B

    // Adjust angular velocity based on the torque generated by the collision
    if !rigid_body_a.fixed {
        rigid_body_a.angular_velocity += r_a / rigid_body_a.moment_of_inertia;
    }

    if !rigid_body_b.fixed {
        rigid_body_b.angular_velocity += r_b / rigid_body_b.moment_of_inertia;
    }
}
        ...
        if check_aabb_collision(
            ...
            resolve_aabb_collision(
                (
                    ...
                    // &mut transform_a.rotation,
                ),
                (
                    &mut collider_b,
                    &mut rb_b,
                    &mut transform_b, // &mut transform_b.translation,
                                      // &mut transform_b.rotation,
                ),
            );
        }
    }
...
fn apply_physics(mut query: Query<(&mut RigidBody, &mut Transform)>, time: Res<Time>) {
    for (mut rigid_body, mut transform) in query.iter_mut() {
        ...
        rigid_body.apply_linear_velocity(&mut transform.translation, time);
        rigid_body.apply_angular_velocity(&mut transform.rotation, time);
        rigid_body.apply_gravity(&mut transform.translation, time);
    }
}
